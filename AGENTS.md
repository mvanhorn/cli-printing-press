...
| **refresh-docs** | CLI-driven regeneration of `README.md` and `SKILL.md` for an already-shipped printed CLI. Reads the Cobra command tree from source (via `internal/docparse`) as the ground truth for what commands exist, classifies them as base vs. transcendence using git history + the `.printing-press.json` timestamp, and renders docs through `internal/docrefresh` templates. Never modifies source code — enforced by safety-invariant tests. Subcommand: `printing-press refresh-docs`. Use it retroactively on pre-#186 CLIs (to pick up the new narrative-doc machinery without losing post-gen transcendence) and after emboss/polish cycles (to keep docs in sync with evolving code). |
