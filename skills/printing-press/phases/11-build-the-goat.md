...
8. **Verify-friendly RunE**: Hand-written commands MUST NOT use `Args: cobra.MinimumNArgs(N)` or `MarkFlagRequired(...)`. Cobra evaluates both before RunE runs, so a `--dry-run` guard inside RunE cannot reach if those gates fail. Verify probes commands with `--dry-run` and expects exit 0; commands with hard arg/flag gates fail those probes. Instead: validate inside RunE, fall through to `cmd.Help()` only for unambiguous help-only invocations (no args and no flags), short-circuit on `dryRunOK(flags)` before any IO, and return `usageErr(...)` with exit 2 when required input is missing in real mode.
   ...
   - **Declare verifier fixture inputs when generic values are not enough**: generated endpoint commands synthesize `pp:happy-args` from spec `example` / `enum` / `default` / `format` when every required input is derivable, and they omit `example-value` from `Example:` rather than shipping a known-broken copy-paste. Hand-author the annotation for novel commands, or for endpoint mirrors whose required inputs are genuinely underivable (opaque IDs, coordinates with no schema hint). Add `Annotations: map[string]string{"pp:happy-args": "<item>=example-id;--query=example;--dry-run"}` or assign a whole initialized `cmd.Annotations` map after construction. The verifier consumes tokens separated by unescaped semicolons in order: escape a literal semicolon as `\;`. Positional tokens are `label=value` pairs — the label is discarded and the value becomes the positional, so `<item>=example-id` and `item=example-id` behave identically. **Bare positional values without `=` are silently ignored**, so write `item=example-id`, never just `example-id`; a missing `=` produces no positional fixture and the happy path silently skips. `--flag=value` tokens replace matching Example flags or add new flag/value pairs; bare `--flag` tokens are treated as boolean `--flag=true`. Negative numeric flag values use the `--flag=-12.3` form. Commands without the annotation keep the generic synthesized inputs.
   - **Local-store novel commands need a happy path that exits 0 to pass publish.** Publish's phase5 gate rejects novel features that never reached a passing live-dogfood happy path ("hollow coverage"), and only a non-`--dry-run` `happy_path` check with exit 0 counts. So a local-only command (e.g. `entity report`, `monitor diff`) needs happy-path inputs it can actually satisfy: declare `Annotations: map[string]string{"pp:happy-args": "<name>=example"}` so the matrix has a real positional, and have the command report an empty local store as an ordinary exit-0 result (the shape `learnings stats` uses on a fresh print) rather than a graceful not-found exit. `pp:typed-exit-codes` is not a substitute: the live matrix records a declared non-zero happy path as a skip with reason `declared non-zero exit <code>`, which keeps it out of the failure column but leaves the feature hollow, so `publish validate` still fails with "phase5 acceptance has hollow coverage" and the whole publish loop restarts.
...
- `filterFields(data json.RawMessage, fields string) json.RawMessage` - apply `--select` to a JSON blob; unmatched paths warn on stderr. Prefer `printJSONFiltered` / `printOutputWithFlags` so a total miss can exit non-zero after writing JSON.
- `filterFieldsChecked(data json.RawMessage, fields string) (json.RawMessage, error)` - same projection, but returns a usage error when every requested path matched nothing. Partial misses and empty collections stay non-fatal.
...
**Authenticated novel requests:** Prefer `flags.newClient()` so generated transport applies `config.Load` + `AuthHeader()`. If a novel command builds a raw `http.Request`, set the spec header from `novelAuthHeader(flags)` (or `config.Load(flags.configPath)` + `cfg.AuthHeader()`). Never `os.Getenv("<AUTH_ENV>")` for that header: `auth login` / `auth set-token` persist the credentials file, and getenv misses it. Check `resp.StatusCode` before decoding the body; map 401/403 through `classifyAPIErrorOnly` so a plain-text auth failure is not reported as a JSON syntax error.
