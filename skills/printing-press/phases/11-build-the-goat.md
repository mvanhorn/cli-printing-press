...
- `filterFields(data json.RawMessage, fields string) (json.RawMessage, error)` - apply `--select` to a JSON blob; unmatched paths warn on stderr. Returns a usage error when every requested path matched nothing. Partial misses and empty collections stay non-fatal. Prefer `printJSONFiltered` / `printOutputWithFlags`, which write JSON first and then surface that error.
