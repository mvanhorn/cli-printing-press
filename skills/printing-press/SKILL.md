...
if [ -d "$LIB_TARGET" ] && [ -f "$LIB_TARGET/.printing-press.json" ]; then
  ...
  if [ "$HAS_NOVEL_FIELD" = "true" ]; then
    ...
    # File probe fallback: any *.go file under internal/cli/, internal/syncer/,
    # or internal/store/ whose first 20 lines lack the "Generated by CLI
    # Printing Press" header is hand-authored. One such file is enough.
...
The presence check (`jq 'has("novel_features")'`) is what makes the fallback reachable: without it, `null // [] | length` returns 0 for an absent field and is indistinguishable from a manifest that explicitly records zero novels, silently routing older CLIs through the destructive Path A swap.
