...
The umbrella defaults to `verify --fix` (auto-repair common failures), `validate-narrative --strict --full-examples` (README/SKILL narrative command validation), and `scorecard --live-check` (sample novel-feature output against real targets). When Go sources under `cmd/<cli>/` or `internal/` are newer than `build/stage/bin/<cli>`, `scorecard --live-check` rebuilds the staged binary before sampling and reports the refresh action in human and JSON output. Use `--no-fix` for a read-only pass, `--no-live-check` to skip live sampling, or `--json` for a structured envelope (suppresses per-leg output for clean piping). Pass `--env-var NAME` when live testing needs a credential: a non-empty named variable selects live verification without putting its value in process arguments. An unset or empty named variable falls back to mock mode with a diagnostic in `mode_detail` and human output. Explicit `--api-key` retains precedence. Use `--strict` to make verify-skill treat likely-false-positive findings as failures.
