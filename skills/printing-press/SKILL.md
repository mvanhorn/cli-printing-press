...
NOVEL_COUNT=0
if [ -d "$LIB_TARGET" ] && [ -f "$LIB_TARGET/.printing-press.json" ]; then
  NOVEL_COUNT=$(jq -r '.novel_features // [] | length' "$LIB_TARGET/.printing-press.json" 2>/dev/null || echo 0)
fi
```

`novel_features` is the canonical manifest-recorded count. If the field is absent (older CLI predating the field) and the library exists, fall back to a file probe: any `*.go` file under `$LIB_TARGET/internal/cli/`, `$LIB_TARGET/internal/syncer/`, or `$LIB_TARGET/internal/store/` whose first 20 lines lack `Generated by CLI Printing Press` should be treated as hand-authored. Treat one such file as `NOVEL_COUNT > 0`.
...
cli-printing-press regen-merge "$LIB_TARGET" \
  --fresh "$CLI_WORK_DIR" --apply
```

After `regen-merge` succeeds, the live library directory is the new run. Do **not** then call `lock promote --dir "$CLI_WORK_DIR"` — that would atomically swap the working dir over the just-merged library and undo the preservation. Promote in place: point `lock promote --dir` at the library itself so the manifest write, run-pointer update, and lock release still run:

```bash
cli-printing-press lock promote --cli <api>-pp-cli --dir "$LIB_TARGET"
```

If `regen-merge` reports any `TEMPLATED-WITH-ADDITIONS` files or merge conflicts, stop and surface them to the user before promoting — those represent inline hand-edits to generator-emitted files that need human review (see lines 2823–2833 for the separate-file pattern that avoids this in future). The dry-run report (omit `--apply`) is the right tool for inspection.
