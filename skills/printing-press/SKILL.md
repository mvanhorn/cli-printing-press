...
If the command reads a file or directory (`os.ReadFile`, `os.ReadDir`, `os.Stat`), the read MUST come after `dryRunOK()`, not before. Filesystem reads before `dryRunOK()` cause `validate-narrative --full-examples` to fail with a missing-file error rather than a clean dry-run exit 0.
