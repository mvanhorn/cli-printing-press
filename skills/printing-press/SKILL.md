...
12. **Don't put side-effectful auth setup in `quickstart[0]` for cookie/composed/session/oauth auth.** If `auth.type` is one of {cookie, composed, session_handshake, oauth2}, do NOT place the first auth-setup invocation in `narrative.quickstart[0]`. `validate-narrative --strict --full-examples` classifies `auth login`, `auth set-token`, etc. as side-effectful (auth/launch/apply) and rejects them under strict mode, so authoring step 0 as `auth login --chrome` guarantees a Phase 4 shipcheck round-trip. Use `doctor` (or another read-only invocation like `account get` if auth-required-and-cookie-already-imported is the onboarding entry point) as the first quickstart step, and mention the one-time `auth login --chrome` step in that step's `comment` field. The auth flow itself belongs in `auth_narrative` prose, not in `quickstart`. The rule is auth-type-conditional, not a blanket ban — `api_key` and `bearer_token` quickstarts starting with `auth set-token` are still allowed because the side-effect classifier doesn't reject them there.
