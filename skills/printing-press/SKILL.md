...
**STOP — the enrichment procedure is NOT in this file. You MUST `Read` [references/phase-2-enrichments.md](references/phase-2-enrichments.md) now and apply its four decisions to the resolved spec *before* the Lock-and-Generate invocation below.** The four decisions are **Category**, **Cache**, **Auth**, and **MCP surface** — but the criteria for each (how to choose, when a catalog run skips it, which auth mode to emit) live only in the reference. Deciding them from memory or from these four words instead of reading the reference is the known lazy-load failure: auth, cache, and category get bolted on *after* generation, which forces hand-patching generated files, causes `verify-skill canonical-sections` drift, and ships a manifest that misreports the CLI. These decisions shape the spec/flags that drive `generate`; there is no correct way to run Lock and Generate without first reading the reference and applying them. Do not proceed until you have.
