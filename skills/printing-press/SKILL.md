...
- **Custom request headers** (vendor fingerprint, `X-CSRF`, app-version, signed timestamps): create `internal/client/<api>_headers.go` exporting a func your novel code calls before each request. Do not edit the templated header block in `client.go`.
- **Custom auth flow** (browser-sniffed sessions, vendor SSO, refresh hooks beyond OAuth2): create `internal/auth/<api>_auth.go` with the API-specific token capture or refresh and wire it from a novel command rather than the templated method switch in `auth.go`.
...
- **New novel command:** put the command in its own `internal/cli/<feature>.go` file. Commands declared in the spec are picked up automatically by the generator's typed-tool path and survive regen without a hand-wired `AddCommand` in `root.go`.

If the only way to wire an extension is to edit an emitted file (an `AddCommand` call with no registry hook, a `case` branch in a method switch), file a generator issue rather than editing inline. Restoring the same inline change on every regen is the bug, not your workflow.
