  ...
  users:                          # resource key becomes top-level command: <name>-cli users
    ...
    endpoints:                    # map[string]Endpoint (REQUIRED: at least one key)
      list:                       # endpoint key becomes subcommand: users list
        ...
        example: "my-api-pp-cli users list --limit 25" # string optional Cobra Example override
...
`--help`; include the generated binary name and command path. Set `happy_args`
when live dogfood needs realistic arguments that cannot be inferred from names
or schema hints. The value is copied into the generated `pp:happy-args`
