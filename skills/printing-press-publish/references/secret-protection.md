> **⚠ DRIFT NOTICE — do not edit this copy in place.** This is a local copy for the
> `printing-press-publish` skill only. The canonical source of truth is
> `skills/printing-press/references/secret-protection.md` in this repo.
> The two files are intentionally duplicated so the publishing skill stays
> self-contained (the Codex skill registry forbids cross-skill `..` resource links).
> Edit the **BASE** file here, then re-sync with
> `bin/update-skills.sh --printing-press-only`; the next sync overwrites this copy.
