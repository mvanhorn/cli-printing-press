...
if command -v gosec >/dev/null 2>&1; then
  ...
  go run github.com/securego/gosec/v2/cmd/gosec@latest -fmt=json -out=/tmp/polish-gosec-before.json ./... 2>&1 || true
fi
```
...
if command -v gosec >/dev/null 2>&1; then
  ...
  go run github.com/securego/gosec/v2/cmd/gosec@latest -fmt=json -out=/tmp/polish-gosec-after.json ./... 2>&1 || true
fi
```
...
**`gosec_*` values.** Emit the count of gosec findings that are still relevant
to this printed CLI after generated-file triage. Do not count generator-emitted
findings that were routed to `skipped_findings` as retro candidates, but do
count unresolved hand-authored findings that force `hold`.
