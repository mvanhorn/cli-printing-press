...
alter command behavior or recall's verified results on their own.`,
		Example: `  learn-loop-example-pp-cli learnings candidates
  learn-loop-example-pp-cli learnings candidates --class flag_alias
  learn-loop-example-pp-cli learnings candidates --status rejected`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return writeDryRun(flags, "learnings candidates")
			}
...
Usage errors (unknown id, non-open status) exit 2.`,
		Example: `  learn-loop-example-pp-cli learnings confirm 3`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return writeDryRun(flags, "learnings confirm")
			}
...
Usage errors (unknown id, no-reject-path candidates) exit 2.`,
		Example: `  learn-loop-example-pp-cli learnings reject 3`,
		// Usage errors here exit 2 by design; declare it so
		// `cli-printing-press verify` scores the Execute cell honestly.
		Annotations: map[string]string{"pp:typed-exit-codes": "0,2"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return writeDryRun(flags, "learnings reject")
			}
			id, err := parseCandidateID(args)
			if err != nil {
			...
			if errors.Is(err, store.ErrConfirmedFlagAliasReject) {
				return usageErr(fmt.Errorf("learnings reject: candidate %d is a confirmed flag correction with no reject path; repair the guidance with \"learn-loop-example-pp-cli playbook amend\" instead", id))
			}
			if err != nil {
				return usageErr(fmt.Errorf("learnings reject: %w", err))
			}
...
purge also clears stale candidates nobody judged.`,
		Example: `  learn-loop-example-pp-cli learnings purge
  learn-loop-example-pp-cli learnings purge --tombstones`,
		Annotations: map[string]string{"mcp:hidden": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return writeDryRun(flags, "learnings purge")
			}
			s, err := store.OpenWithContext(cmd.Context(), learnDBPath(dbPath))
			if err != nil {
