...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dryRunOK(flags) {
				return writeDryRun(flags, "teach")
			}
...
when learnings exist.`,
  ...
  learn-loop-example-pp-cli recall "<question>" --agent --min-confidence 2`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dryRunOK(flags) {
				return writeDryRun(flags, "recall")
			}
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
  ...
  learn-loop-example-pp-cli learnings list --warnings --agent`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return writeDryRun(flags, "learnings list")
			}
			if warningsOnly {
...
Requires at least one of --resource, --action, or --all.`,
  ...
  learn-loop-example-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dryRunOK(flags) {
				return writeDryRun(flags, "learnings forget")
			}
			query := strings.Join(args, " ")
			dbPath = learnDBPath(dbPath)
			s, err := store.OpenWithContext(cmd.Context(), dbPath)
...
a whole family.`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return writeDryRun(flags, "teach-pattern")
			}
			if noLearnActive(flags) {
				return nil
			}
...
cannot be taught — they are derived from the canonical input.`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return writeDryRun(flags, "teach-lookup")
			}
			if noLearnActive(flags) {
				return nil
			}
