...
func newTicketsPromotedCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cmd.Flags().Changed("max-records") || bodyMaxRecords != 0 {
				bodyMap["MaxRecords"] = bodyMaxRecords
			}
			if cmd.Flags().Changed("filter") || bodyFilter != "" {
				...
				asArray, ok := parsedFilter.([]any)
				if !ok {
					return fmt.Errorf("--filter must be a JSON array, got JSON %T", parsedFilter)
				}
				bodyMap["filter"] = asArray
			}
