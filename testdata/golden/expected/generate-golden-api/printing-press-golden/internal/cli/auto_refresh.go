...
var readCommandResources = map[string][]string{
	"printing-press-golden-pp-cli currencies":        {"currencies"},
	"printing-press-golden-pp-cli currencies list":   {"currencies"},
	"printing-press-golden-pp-cli currencies get":    {"currencies"},
	"printing-press-golden-pp-cli currencies search": {"currencies"},
	"printing-press-golden-pp-cli projects":          {"projects"},
...
func cachePolicy() cliutil.Policy {
	...
	envOptOut := "PRINTING_PRESS_GOLDEN_NO_AUTO_REFRESH"
...
func autoRefreshIfStale(ctx context.Context, flags *rootFlags, resources []string) (meta cliutil.FreshnessMeta) {
	...
	if err := runAutoRefresh(refreshCtx, flags, db, resources); err != nil {
		fmt.Fprintf(os.Stderr, "warning: using stale printing-press-golden-pp-cli cache (refresh failed: %v)\n", err)
