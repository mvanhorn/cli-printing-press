...
// non-GET request. The cache TTL is 5 minutes and keys are opaque
// sha256(path,params)[0:8] — selective invalidation by resource family
// would require a cache-format change. Worst case after a nuke is one
// extra GET on the next read.
// See docs/solutions/design-patterns/http-client-cache-invalidate-on-mutation-2026-05-05.md
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if resp.StatusCode < 400 {
			c.limiter.OnSuccess()
			// Cache-invalidate on successful non-GET requests. The !c.DryRun
			// guard is structurally redundant (dry-run short-circuits before
			// the retry loop) but defends against future refactors.
