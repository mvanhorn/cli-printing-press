...
func TestCorruptCredentialsFallsBackToEnvCredential(t *testing.T) {
	...
	t.Setenv("PRINTING_PRESS_OAUTH2_OAUTH2_AUTH_CODE", "env-secret")
	credentialsPath := filepath.Join(home, ".local", "share", "printing-press-oauth2-pp-cli", "credentials.toml")
	if err := os.MkdirAll(filepath.Dir(credentialsPath), 0o700); err != nil {
		t.Fatalf("mkdir credentials dir: %v", err)
	}
...
func TestAuthWriteScrubsLegacyConfigWhenRelocated(t *testing.T) {
	...
	activeConfigPath := filepath.Join(newConfigDir, "printing-press-oauth2-pp-cli", "config.toml")
	...
	credsPath := filepath.Join(newDataDir, "printing-press-oauth2-pp-cli", "credentials.toml")
