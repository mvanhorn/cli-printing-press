...
//
// For RPC-style POST responses that wrap items in a common envelope key
// (results, data, items, hits) and pad the envelope with heavy companion
// fields (full context blobs, pre-fetched prose, raw_response), --select
// auto-descends into that array when none of the selected paths match a
// top-level key. The user wrote --select id,score expecting item-level
// fields; returning just the narrowed array strips the heavy envelope
// fields, which is the dominant token-saving win on RAG-shaped responses.
// To preserve the envelope wrap, use dotted paths (--select results.id,
// results.score).
func filterFields(data json.RawMessage, fields string) json.RawMessage {
	...
	if inner, ok := selectEnvelopeArray(data, paths); ok {
		return filterFieldsRec(inner, paths)
	}
	return filterFieldsRec(data, paths)
}

// selectEnvelopeArray returns the inner array if `data` is an object whose
// top-level keys don't match any path head AND a common envelope key
// (results, data, items, hits) holds an array. Used by filterFields to make
...
func selectEnvelopeArray(data json.RawMessage, paths [][]string) (json.RawMessage, bool) {
	...
	for _, p := range paths {
		...
		for k := range obj {
			...
			if lower == head || camelToKebab(k) == head {
				return nil, false
			}
		}
	}
