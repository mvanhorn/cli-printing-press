# BLE Desk Lamp Printed CLI Agent Guide

This directory is a generated `ble-desk-lamp-pp-cli` BLE device CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
ble-desk-lamp-pp-cli doctor --json
ble-desk-lamp-pp-cli capabilities --json
```
...
ble-desk-lamp-pp-cli status --live --json
```
...
```bash
ble-desk-lamp-pp-cli <command> --help
ble-desk-lamp-pp-cli <command> --dry-run --json
```
