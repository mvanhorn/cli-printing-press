...
import (
	...
	"io"
	"os"
	"query-endpoint-golden-pp-cli/internal/client"
	"query-endpoint-golden-pp-cli/internal/cliutil"
	"query-endpoint-golden-pp-cli/internal/learn"
	"query-endpoint-golden-pp-cli/internal/learn/lookups"
...
Resource scoping:
  ...
  query-endpoint-golden-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  query-endpoint-golden-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  query-endpoint-golden-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("query-endpoint-golden-pp-cli")
			}
...
func syncResource(ctx context.Context, c interface {
	...
	for {
		...
		// Query-endpoint paging: the shared query endpoint carries its position
		// in-query (STARTPOSITION), not in a body cursor, so extractPageItems
		...
		// Query-endpoint resources drive their own STARTPOSITION paging via the
		// hasMore/nextCursor set above, so they must not break on the generic
...
// queryEntity maps each SQL-query-endpoint resource to its API entity name for
// the shared query endpoint's injected SELECT (e.g. "invoices" -> "Invoice").
// Populated from the spec's query_sync hint plus each list endpoint's response
// item; empty for every non-query API. Drives the query injection, envelope
// unwrap, and offset-paging blocks in syncResource.
var queryEntity = map[string]string{
	"gadgets": "Gadget",
	"widgets": "Widget",
}
