...
Run sync first to populate the local search index.`,
  ...
  public-param-golden-pp-cli search "status"

  # Force local search explicitly
  public-param-golden-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  public-param-golden-pp-cli search "status" --type stores --data-source local
  # JSON output for piping
  public-param-golden-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'public-param-golden-pp-cli sync --resources stores' first to populate the local database.", err)
			}
			defer db.Close()
