...
import (
	...
	"strings"
	"sync"
	"sync-history-filter-pp-cli/internal/cliutil"
	"sync-history-filter-pp-cli/internal/learn"
	"sync-history-filter-pp-cli/internal/learn/lookups"
	"sync-history-filter-pp-cli/internal/store"
	"sync/atomic"
	"time"
)
...
Resource scoping:
  ...
  sync-history-filter-pp-cli sync
  # Sync specific resources only
  sync-history-filter-pp-cli sync --resources archived,issues

  # Full resync (ignore previous checkpoint)
  sync-history-filter-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  sync-history-filter-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  sync-history-filter-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  sync-history-filter-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("sync-history-filter-pp-cli")
			}
...
type hiddenHistoryDefault struct {
	Name  string
	Value string
}

func syncResourceHiddenHistoryDefaults(resource string) []hiddenHistoryDefault {
	switch resource {
	case "orders":
		return []hiddenHistoryDefault{
...
func syncUserParamsOverrides(p *syncUserParams, resource, key string, isDependent bool) bool {
	if p == nil {
		return false
	}
...
func emitDefaultFilterHistoryWarning(resource string, count int, userParams *syncUserParams, isDependent bool, syncEvents io.Writer) error {
	if count != 0 {
		return nil
	}
	var warn error
	for _, item := range syncResourceHiddenHistoryDefaults(resource) {
		if syncUserParamsOverrides(userParams, resource, item.Name, isDependent) {
			continue
		}
		if warn == nil {
			warn = fmt.Errorf("%s synced 0 rows while list default %s=%s hides closed/historical rows; re-run with --param %s=<all-history value> or set sync_params / x-sync-params", resource, item.Name, item.Value, item.Name)
		}
		if humanFriendly {
			continue
		}
		payload := struct {
			...
			Event:    "sync_warning",
			Resource: resource,
			Reason:   "default_filter_hides_history",
			Param:    item.Name,
			Default:  item.Value,
			Message:  fmt.Sprintf("synced 0 rows while list default %s=%s hides closed/historical rows", item.Name, item.Value),
		}
		payloadJSON, _ := json.Marshal(payload)
		fmt.Fprintf(syncEvents, "%s\n", payloadJSON)
	}
...
func responsePathForResource(resource, path string) []string {
	...
	// response_path, generation prefers the collection/sync envelope here;
