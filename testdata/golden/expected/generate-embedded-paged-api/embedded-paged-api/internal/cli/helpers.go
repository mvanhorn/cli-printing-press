...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		...
		hints = append(hints, "Set your API key: export EMBEDDED_PAGED_API_KEY=<your-key>")
		hints = append(hints, "Run 'embedded-paged-pp-cli doctor' to check auth status.")
		return authErr(fmt.Errorf("%w\nhint: %s\n      Response: %s", err, strings.Join(hints, "\n      "), cliutil.SanitizeErrorBody(msg)))
	case strings.Contains(msg, "HTTP 401"):
		hints := []string{"check your API key."}
		hints = append(hints, "Set it with: export EMBEDDED_PAGED_API_KEY=<your-key>")
		hints = append(hints, "Run 'embedded-paged-pp-cli doctor' to check auth status.")
...
func fetchEmbeddedPagedSubresource(c interface {
	...
	for page := 0; page < embeddedPagedSubresourcePageCap; page++ {
		...
		data, _, err := c.GetWithHeaders(target, nil, nil)
