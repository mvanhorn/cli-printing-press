...
import (
	...
	"embedded-paged-pp-cli/internal/config"
	"embedded-paged-pp-cli/internal/platform"
...
func jwtExpirySource(cfg *config.Config) string {
	...
	if v := strings.TrimSpace(cfg.EmbeddedPagedApiKey); v != "" {
		return v
	}
		...
		if !nextIsURL {
			if moreRaw, ok := caseInsensitiveLookup(obj, nextField); ok {
				if hasUnfollowableNextSignal(moreRaw, nextIsBoolean) {
					emitEmbeddedPagedTruncationWarning(ctx, childPath)
				}
			}
			return all, nil
		}
...
func emitEmbeddedPagedTruncationWarning(ctx context.Context, childPath string) {
	platform.MarkContextTruncated(ctx, platform.TruncationReason{Kind: "embedded_subresource_limit", Configured: childPath, Observed: "more_available", MoreAvailable: true})
	if humanFriendly {
...
// mergeCommandMeta keeps keys the command already set. A wrapper source on
// the other provenance axis is kept beside it: transport is live, local, or
// dry-run, and data-origin is catalogue or computed. Those axes are not
// collapsed into one source string.
