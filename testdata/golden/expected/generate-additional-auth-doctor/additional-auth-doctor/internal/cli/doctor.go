...
import (
	...
	"additional-auth-doctor-pp-cli/internal/client"
	"additional-auth-doctor-pp-cli/internal/cliutil"
	"additional-auth-doctor-pp-cli/internal/config"
	"additional-auth-doctor-pp-cli/internal/store"
...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  additional-auth-doctor-pp-cli doctor
  additional-auth-doctor-pp-cli doctor --json
  additional-auth-doctor-pp-cli doctor --fail-on warn
  additional-auth-doctor-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if credentialRefused {
				...
				if cfg != nil {
					...
					if !configured {
						...
						report["auth_hint"] = "Set it with: echo \"$TOKEN\" | additional-auth-doctor-pp-cli auth set-token or export ADDITIONAL_AUTH_DOCTOR_TOKEN=\"your-token-here\""
					} else {
						authConfigured = true
			...
			recordAdditionalAuthEnv := func(name, configuredValue string, required bool) {
				if os.Getenv(name) != "" || configuredValue != "" {
			...
			if os.Getenv("ADDITIONAL_AUTH_DOCTOR_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "ADDITIONAL_AUTH_DOCTOR_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "ADDITIONAL_AUTH_DOCTOR_TOKEN")
			}
			{
				configuredValue := ""
				if cfg != nil {
					configuredValue = cfg.AdditionalAuthDoctorOrganizationId
				}
				recordAdditionalAuthEnv("ADDITIONAL_AUTH_DOCTOR_ORGANIZATION_ID", configuredValue, true)
			}
			{
				configuredValue := ""
				if cfg != nil {
					configuredValue = cfg.AdditionalAuthDoctorAccountLabel
				}
				recordAdditionalAuthEnv("ADDITIONAL_AUTH_DOCTOR_ACCOUNT_LABEL", configuredValue, false)
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "additional-auth-doctor-pp-cli", suggestion)
						} else {
...
func doctorExitForFailOn(failOn string, report map[string]any) error {
	...
	for k, v := range report {
		...
		if ok && !doctorIsInfoKey(k) {
			...
			if strings.HasPrefix(low, "error") || strings.HasPrefix(low, "refused:") || strings.Contains(low, "error") || strings.Contains(low, "unreachable") || strings.Contains(low, "invalid") || strings.Contains(low, "missing") || strings.Contains(low, "rejected") {
				worstError = true
			}
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("additional-auth-doctor-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'additional-auth-doctor-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'additional-auth-doctor-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny:
		...
		report["hint"] = "No sync recorded; run 'additional-auth-doctor-pp-cli sync' to hydrate API-backed resources. Rows written by other paths are not tracked in sync_state."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'additional-auth-doctor-pp-cli sync' to refresh."
	}
	return report
}
