// Copyright 2026 contributors. Licensed under Apache-2.0. See LICENSE.
...
const (
	endpointListNote    = "Typed MCP endpoint response exceeded the tool result budget. Narrow the request with limit, offset, filters, search/sql, or a command-mirror tool with --agent/--compact/--select."
...
func JSON(v any) (string, error) {
	...
	if len(trimmed) > 0 && trimmed[0] == '[' {
		var items []json.RawMessage
		if json.Unmarshal(data, &items) == nil {
			return string(boundedListEnvelope("items", items, len(data), jsonResultNote)), nil
		}
	}
	return previewEnvelope(data, jsonResultNote), nil
}
