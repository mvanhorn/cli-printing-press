...
Use `ble-temperature-sensor-pp-cli capabilities --json` to inspect callable and withheld BLE capabilities, including safety classes and evidence refs. Use `ble-temperature-sensor-pp-cli status --json` to inspect replay-backed status output. Live BLE control and optional session IPC are generated only when later device-session support is enabled by the device spec.
