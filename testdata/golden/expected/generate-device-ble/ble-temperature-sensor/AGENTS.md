# BLE Temperature Sensor Printed CLI Agent Guide

This directory is a generated `ble-temperature-sensor-pp-cli` BLE device CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
ble-temperature-sensor-pp-cli doctor --json
ble-temperature-sensor-pp-cli capabilities --json
```
...
ble-temperature-sensor-pp-cli status --live --json
```
...
```bash
ble-temperature-sensor-pp-cli <command> --help
ble-temperature-sensor-pp-cli <command> --dry-run --json
```
