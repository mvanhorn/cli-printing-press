...
func handleCodeOrchExecute(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
	...
	case "DELETE":
		if len(hdrs) > 0 {
			data, _, err = c.DeleteWithHeaders(path, hdrs)
		} else {
	...
	default:
		...
		case "POST":
			if len(hdrs) > 0 {
				data, _, err = c.PostWithHeaders(path, body, hdrs)
			} else {
				data, _, err = c.Post(path, body)
			}
		case "PUT":
			if len(hdrs) > 0 {
				data, _, err = c.PutWithHeaders(path, body, hdrs)
			} else {
				data, _, err = c.Put(path, body)
			}
		case "PATCH":
			if len(hdrs) > 0 {
				data, _, err = c.PatchWithHeaders(path, body, hdrs)
			} else {
