# BLE Opaque Binary Device Printed CLI Agent Guide

This directory is a generated `ble-opaque-binary-pp-cli` BLE device CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
ble-opaque-binary-pp-cli doctor --json
ble-opaque-binary-pp-cli capabilities --json
```
...
ble-opaque-binary-pp-cli status --live --json
```
...
```bash
ble-opaque-binary-pp-cli <command> --help
ble-opaque-binary-pp-cli <command> --dry-run --json
```
