...
// operations by concatenating path segments. The signal: the operationId,
// snake-cased and tokenized on underscores, contains a token equal to the
// concatenation (no separator) of two consecutive non-routing path
// segments. When true, callers skip operationIDToName so name derivation
// falls back to verb + path-collision-suffix rather than emitting
// unreadable command names like "paymentsubscriptions-get-collection".
//
// The "two consecutive segments concatenated" signal is what distinguishes
// auto-generation from hand-curated ids that merely share the api_ prefix
// (already handled heuristically inside operationIDToName).
func isFrameworkAutoGeneratedOperationID(operationID, path string) bool {
	...
	for _, s := range segs {
		if isVersionSegment(s) || isGenericAPIPrefix(s) {
			continue
		}
		if isPathParamSegment(s) {
			s = strings.Trim(s, "{}")
		}
