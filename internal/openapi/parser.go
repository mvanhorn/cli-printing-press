...
func resolveEndpointName(method, path string, op *openapi3.Operation, existing map[string]spec.Endpoint, resourceName, basePath string, commonPrefix []string) string {
	opID := operationID(op)
	var name string
	if fastAPIName, ok := fastAPIOperationIDEndpointName(opID, path, resourceName, commonPrefix); ok {
		name = fastAPIName
	} else if !isFrameworkAutoGeneratedOperationID(opID, path) {
...
func fastAPIOperationIDEndpointName(operationID, path, resourceName string, commonPrefix []string) (string, bool) {
	if operationID == "" || path == "" {
		return "", false
	}
	opTokens := strings.Split(toSnakeCase(operationID), "_")
	...
	case "create":
		if defaultEndpointName("POST", path) == "create" {
			return "", true
		}
	case "update":
		if defaultEndpointName("PATCH", path) == "update" {
			return "", true
		}
	case "delete":
		if defaultEndpointName("DELETE", path) == "delete" {
			return "", true
		}
	case "read":
		if strings.EqualFold(opTokens[len(opTokens)-1], "get") {
			return "", true
		}
	}
	return name, true
