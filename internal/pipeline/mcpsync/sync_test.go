...
func TestSyncRefusesLegacyClientBeforeRewritingMCP(t *testing.T) {
	t.Parallel()
	...
	apiSpec := &spec.APISpec{
		...
		Config: spec.ConfigSpec{
			Format: "toml",
			Path:   "~/.config/legacyclient-pp-cli/config.toml",
		},
	...
	cliDir := filepath.Join(t.TempDir(), "legacyclient")
	...
	require.NoError(t, err)
	legacyClient := strings.Replace(string(clientData),
		"func (c *Client) Get(ctx context.Context, path string, params map[string]string)",
		"func (c *Client) Get(path string, params map[string]string)", 1)
	require.NotEqual(t, string(clientData), legacyClient)
	require.NoError(t, os.WriteFile(clientPath, []byte(legacyClient), 0o644))

	toolsPath := filepath.Join(cliDir, "internal", "mcp", "tools.go")
	legacyTools := []byte(`// Generated by CLI Printing Press (https://github.com/mvanhorn/cli-printing-press). DO NOT EDIT.
package mcp

func RegisterNovelFeatureTools() { shellOutToCLI("items list") }
`)
	require.NoError(t, os.WriteFile(toolsPath, legacyTools, 0o644))
	...
	after, readErr := os.ReadFile(toolsPath)
...
func TestEnsureMCPClientSurfaceCompatibleChecksConditionalRequestTypes(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			method:             "POST",
			missingName:        "PostQueryFormWithParams",
			missingMarker:      "func (c *Client) PostQueryFormWithParams(ctx context.Context,",
		},
		{
			name:               "multipart",
			requestContentType: "multipart/form-data",
			method:             "PUT",
			missingName:        "PutMultipartWithParams",
			missingMarker:      "func (c *Client) PutMultipartWithParams(ctx context.Context,",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			apiSpec := &spec.APISpec{
				...
				Config: spec.ConfigSpec{
					Format: "toml",
					Path:   "~/.config/" + tt.name + "client-pp-cli/config.toml",
				},
				Resources: map[string]spec.Resource{
					"items": {
						Endpoints: map[string]spec.Endpoint{
							"create": {
								Method:             tt.method,
								Path:               "/items/{id}",
								RequestContentType: tt.requestContentType,
								Body: []spec.Param{
			...
			cliDir := filepath.Join(t.TempDir(), apiSpec.Name)
			require.NoError(t, generator.New(apiSpec, cliDir).Generate())

			clientPath := filepath.Join(cliDir, "internal", "client", "client.go")
			clientData, err := os.ReadFile(clientPath)
			require.NoError(t, err)
			incompatible := strings.Replace(string(clientData), tt.missingMarker, "func (c *Client) Legacy"+tt.missingName+"(ctx context.Context,", 1)
			require.NotEqual(t, string(clientData), incompatible)
			require.NoError(t, os.WriteFile(clientPath, []byte(incompatible), 0o644))

			err = ensureMCPClientSurfaceCompatible(cliDir)
			require.Error(t, err)
			assert.Contains(t, err.Error(), tt.missingName)
			assert.Contains(t, err.Error(), "reprint required")
		})
	}
}
