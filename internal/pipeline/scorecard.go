...
// A list+detail GET pair (P and P/{param}) is exactly the shape the
// generator's profiler turns into a syncable store; RPC-style action
// endpoints (/v1/load, /v1/sql, /v1/meta) have no such pairing. Callers must
// pre-filter to GET paths, or a write-only P + P/{param} pair would look
// readable.
...
// the profiler dropped it, so the scorecard scores it 0 rather than N/A-ing
// it into silence.
//
