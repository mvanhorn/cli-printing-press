...
func TestApplyEbayAuthFixturePreservesAuth(t *testing.T) {
	...
	require.NoError(t, err)
	got := string(authAfter)
	for _, name := range []string{
		"pkceChallenge", "oauthBrowserFlow", "persistToken",
		"refreshTokenIfNeeded", "authenticateWithCookie",
	} {
		assert.Contains(t, got, name, "hand-added OAuth function %s must survive overlay", name)
	}
	assert.Contains(t, got, "crypto/sha256")
	assert.Contains(t, got, "func newAuthCmd")
}
...
func TestApplyTakesFreshTemplatedBodiesAndKeepsHandAdditions(t *testing.T) {
	t.Parallel()

	header := "// Generated by CLI Printing Press. DO NOT EDIT.\npackage client\n\n"
	gomod := "module example.com/body-takeover\n\ngo 1.22\n"
	stagedPubDir, freshDir := stagedSyntheticPair(t,
		map[string]string{
			"go.mod": gomod,
			"internal/client/client.go": header + `func credentialAppliesToURL(u string) bool { return u != "" }
func HandAdded() string { return "kept" }
`,
		},
		map[string]string{
			"go.mod": gomod,
			"internal/client/client.go": header + `func credentialAppliesToURL(u string) bool { return u == "https://bound.example" }
`,
		})
	...
	got := readFileString(t, filepath.Join(stagedPubDir, "internal", "client", "client.go"))
	assert.Contains(t, got, `u == "https://bound.example"`,
		"Apply must take the fresh templated body even when the file has a hand addition")
	assert.NotContains(t, got, `u != ""`)
	assert.Contains(t, got, `return "kept"`,
		"hand-authored novel decls must survive Apply")
}

func TestApplyTakesFreshBodiesOnBodyDriftWithoutBase(t *testing.T) {
	t.Parallel()

	header := "// Generated by CLI Printing Press. DO NOT EDIT.\npackage store\n\n"
	gomod := "module example.com/store-drift\n\ngo 1.22\n"
	stagedPubDir, freshDir := stagedSyntheticPair(t,
		map[string]string{
			"go.mod": gomod,
			"internal/store/store.go": header + `func ResolveStorageID(id string) string { return oldHelper(id) }
`,
		},
		map[string]string{
			"go.mod": gomod,
			"internal/store/store.go": header + `func ResolveStorageID(id string) string { return newHelper(id) }
`,
		})

	report, err := Classify(stagedPubDir, freshDir, Options{Force: true})
	require.NoError(t, err)
	require.Equal(t, VerdictTemplatedBodyDrift, verdictMap(report)["internal/store/store.go"])
	require.NoError(t, Apply(report, Options{Force: true}))

	got := readFileString(t, filepath.Join(stagedPubDir, "internal", "store", "store.go"))
	assert.Contains(t, got, "newHelper")
	assert.NotContains(t, got, "oldHelper")
}

func TestApplyRefreshesPrintingPressManifestProvenance(t *testing.T) {
	t.Parallel()

	root := `// Generated by CLI Printing Press. DO NOT EDIT.
package cli

func Execute() {}
`
	gomod := "module example.com/manifest-refresh\n\ngo 1.22\n"
	stagedPubDir, freshDir := stagedSyntheticPair(t,
		map[string]string{
			".printing-press.json": `{"schema_version":1,"printing_press_version":"4.31.2","run_id":"old-run","owner":"matt-van-horn","cli_name":"x-pp-cli","api_name":"x"}`,
			"go.mod":               gomod,
			"internal/cli/root.go": root,
		},
		map[string]string{
			".printing-press.json": `{"schema_version":2,"printing_press_version":"4.31.5","run_id":"new-run","owner":"trevin-chow","cli_name":"x-pp-cli","api_name":"x"}`,
			"go.mod":               gomod,
			"internal/cli/root.go": root + "\nfunc newFreshCmd() {}\n",
		})
	...
	got := readFileString(t, filepath.Join(stagedPubDir, ".printing-press.json"))
	assert.Contains(t, got, `"4.31.5"`)
	assert.Contains(t, got, `"new-run"`)
	assert.Contains(t, got, `"matt-van-horn"`)
	assert.NotContains(t, got, `"4.31.2"`)
	assert.NotContains(t, got, `"old-run"`)
	assert.NotContains(t, got, `"trevin-chow"`)
}
