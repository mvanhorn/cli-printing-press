...
func TestMergeIntoFreshTreeKeepsHandEditedFuncWhenFreshIsBase(t *testing.T) {
	t.Parallel()
	...
	write(snap, `func Shared() string { return "kept" }
`)
	write(fresh, `func Shared() string { return "fresh" }
`)

	report, err := Classify(snap, fresh, Options{Force: true, BaseDir: fresh})
	require.NoError(t, err)
	require.NoError(t, MergeIntoFreshTree(snap, fresh, report, Options{Force: true, BaseDir: fresh}))

	got, err := os.ReadFile(filepath.Join(fresh, rel))
	require.NoError(t, err)
	assert.Contains(t, string(got), `return "kept"`,
		"same-version force must keep a hand-edited generated function body")
	assert.NotContains(t, string(got), `return "fresh"`)
	...
	assert.True(t, applied, "preserving the hand-edit must count the file as applied")
}
