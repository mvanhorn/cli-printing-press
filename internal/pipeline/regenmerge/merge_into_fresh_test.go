...
`)
	...
	require.NoError(t, err)
	assert.NotContains(t, string(gotVersion), "var version",
		"fresh split-out version.go must not redeclare a var preserved in root.go")
	assert.Contains(t, string(gotVersion), "func newVersionCmd",
		"non-colliding fresh declarations must survive")
