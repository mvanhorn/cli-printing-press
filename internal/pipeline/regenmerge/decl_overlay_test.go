...
func TestOverlayKeepsHandEditedFuncWhenBaseIsFresh(t *testing.T) {
	t.Parallel()
...
`)
	dest := filepath.Join(dir, "dest.go")
	require.NoError(t, overlayHandEditedDecls(pub, fresh, fresh, dest))

	got, err := os.ReadFile(dest)
	require.NoError(t, err)
	src := string(got)
	assert.Contains(t, src, `return "kept"`,
		"same-version force treats fresh as the original emission, so a differing shared body overlays")
	assert.NotContains(t, src, `return "fresh"`)
}
